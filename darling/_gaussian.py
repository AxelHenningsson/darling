...
@numba.jit(nopython=True)
def adam_update(param, m, v, grad, t, alpha=0.001, beta1=0.9, beta2=0.999, epsilon=1e-8):
...
@numba.jit(nopython=True)
def fit_gaussian_2d(data_flat, x_flat, y_flat, stat_mean, stat_cov, max_iter=1000, tolerance=1e-3):
    ...
    for _ in range(max_iter):
        ...
        for i in range(len(data_flat)):
            ...
            grad[1] += residual * params[0] * exp_term * (dx / (sigma_x ** 2) - rho * dy / (sigma_x * sigma_y)) / (1 - rho ** 2)
            grad[2] += residual * params[0] * exp_term * (dy / (sigma_y ** 2) - rho * dx / (sigma_x * sigma_y)) / (1 - rho ** 2)
            grad[3] += residual * params[0] * exp_term * dx ** 2 / (sigma_x ** 3) / (1 - rho ** 2)
            grad[4] += residual * params[0] * exp_term * dy ** 2 / (sigma_y ** 3) / (1 - rho ** 2)
            grad[5] += residual * params[0] * exp_term * (rho / (1 - rho ** 2) - dx * dy / (sigma_x * sigma_y))
        ...
        if active_points > 0:
            ...
            if t > 3:
                error_trend = (error_history[(error_idx - 1) % 3] - error_history[error_idx % 3]) / error_history[error_idx % 3]
            else:
                error_trend = -1.0
            ...
            shape_error = abs(params[3] - best_params[3]) / best_params[3] + abs(params[4] - best_params[4]) / best_params[4]
            if shape_error > 0.1:
                alphas[3:5] *= 1.2
...
def moments_2d_gaussian(data, x, y, dum, stat_mean, stat_cov, res):
    ...
    params = fit_gaussian_2d(data.flatten(), x.flatten(), y.flatten(), stat_mean, stat_cov)
    res[0] = params[1]  # x0
    res[1] = params[2]  # y0
...
def covariance_2d_gaussian(data, first_moments, points, dum, stat_cov, res):
    ...
    params = fit_gaussian_2d(data.flatten(), points[0], points[1], first_moments, stat_cov)
