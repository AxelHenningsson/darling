...
def _asset_path(*parts):
    ref = files("darling").joinpath("_data", *parts)
...
def _get_asset_abspath(*parts):
    return str(files("darling").joinpath("_data", *parts).resolve())
...
def domains(scan_id="1.1"):
    ...
    reader = darling.io.reader.MosaScan(data_path)
...
def rocking_scan():
    ...
    reader = darling.io.reader.RockingScan(data_path)
    dset = darling.DataSet(reader)
